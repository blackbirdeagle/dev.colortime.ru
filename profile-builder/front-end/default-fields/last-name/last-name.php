...
function wppb_last_name_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
			<div class = "business__form__field__left"><label for="last_name">'.$item_title.$error_mark.'</label></div>
			<div class = "business__form__field__right"><input class="text-input default_field_lastname '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .'" name="last_name" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" id="last_name" value="'. esc_attr( wp_unslash( $input_value ) ) .'" '. $extra_attr .'/></div>';
